...
function routeError(error: unknown) {
  ...
  return NextResponse.json({
    error: 'JOB_ROUTE_ERROR',
    message: error instanceof Error ? error.message : 'No fue posible operar la cola de jobs.',
  }, { status: 500 });
}
...
export async function POST(request: NextRequest, context: { params: Promise<{ auditId: string }> }) {
  try {
    ...
    if (action === 'EXTRACT_FACTS') {
      ...
      const frozenRun = existingRuns.find((run) => run.state === 'FROZEN');
      if (frozenRun) return NextResponse.json({ job: null, factRun: frozenRun, reused: true }, { status: 200 });

      const activeRun = existingRuns.find((run) => run.state === 'PROCESSING' || run.state === 'DRAFT');
      const run = activeRun ?? await factsRepo.createRun({ auditId, policyCode: 'GDM_GAM_PRD_MLG_003', policyVersion: '5', extractorVersion: 'deterministic-facts-v1', artifactSetFingerprint: await factsRepo.artifactSetFingerprint(auditId), createdBy: user.id });
