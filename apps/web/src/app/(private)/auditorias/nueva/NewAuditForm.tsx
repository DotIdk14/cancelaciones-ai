      ...
      <div className="grid gap-4 lg:grid-cols-[minmax(0,1.8fr)_minmax(320px,0.9fr)]">
        <div className="rounded-xl border border-line bg-surface-1 p-4">
          <label className="flex min-h-[185px] cursor-pointer flex-col items-center justify-center rounded-xl border border-dashed border-line bg-background px-5 py-6 text-center hover:border-brand/60 hover:bg-surface-2">
            ...
            <p className="mt-1.5 max-w-2xl text-sm text-muted">La validación, OCR y transcripción se ejecutan con los archivos reales del expediente.</p>
        ...
        <div className="overflow-hidden rounded-xl border border-line bg-surface-1">
          <div className="border-b border-line px-5 py-4">
            ...
            <p className="mt-1 text-sm text-muted">Solo se muestran archivos seleccionados para esta nueva auditoría.</p>
          </div>
            ...
            {files.length === 0 ? (
              ...
              <div key={`${file.name}-${file.size}-${file.lastModified}`} className="grid gap-4 px-5 py-4 md:grid-cols-[52px_1fr_140px_120px] md:items-center">
                ...
                <div>
                  ...
                  <p className="mt-1 text-sm text-muted">{formatBytes(file.size)} · Pendiente de subir</p>
                </div>
                <div className="h-2 rounded-full bg-background"><div className="h-2 rounded-full bg-muted" style={{ width: '0%' }} /></div>
                <StatusBadge>0% · En espera</StatusBadge>
              </div>
            ))}
          </div>
          <div className="flex flex-col justify-between gap-4 border-t border-line bg-surface-2 px-5 py-4 xl:flex-row xl:items-center">
            <div className="text-sm text-muted">
              ...
              <p className="mt-1 text-xs text-subtle">El destino se genera automáticamente al crear la auditoría.</p>
            </div>
