...
export function AuditWorkflow({ auditId, factRunId, skipAutoResume }: { auditId: string; factRunId?: string; skipAutoResume?: boolean }) {
  ...
  const refreshQueue = useCallback(async () => {
    ...
    if (!response.ok) throw new Error(data.message ?? 'No fue posible consultar la cola.');
  ...
  const waitForJobs = useCallback(async () => {
    for (let attempt = 0; attempt < 60; attempt += 1) {
      ...
      if (!processResponse.ok) {
        ...
        throw new Error(processData.message ?? 'No fue posible ejecutar el siguiente job.');
      }
  ...
  const run = useCallback(async (files: File[], rerun = false) => {
    ...
    try {
      if (!rerun) {
        ...
        if (!uploadResponse.ok && uploadResponse.status !== 207) throw new Error(uploadData.message ?? 'No fue posible subir la evidencia.');
  ...
  return (
    <section className="rounded-3xl border border-brand/20 bg-white p-6 shadow-sm">
      <div className="flex items-start justify-between gap-4">
        ...
        <span className="rounded-full bg-emerald-50 px-3 py-1 text-xs font-semibold text-emerald-700">{state === 'idle' ? 'Listo' : state === 'done' ? 'Completado' : 'En curso'}</span>
      </div>
      <button
        type="button"
